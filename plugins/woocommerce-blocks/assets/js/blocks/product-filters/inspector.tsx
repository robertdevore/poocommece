...
export const Inspector = ( { attributes, setAttributes }: EditProps ) => {
	...
	return (
		<InspectorControls>
			<PanelBody title={ __( 'Display on mobile', 'poocommerce' ) }>
				<RadioControl
					selected={ overlayButtonType }
					options={ [
						{
							label: __( 'Label and icon', 'poocommerce' ),
							value: 'label-icon',
						},
						{
							label: __( 'Label only', 'poocommerce' ),
							value: 'label-only',
						},
						{
				...
				{ overlayButtonType !== 'label-only' && (
					...
					>
						<ToggleGroupControlOption
							value={ 'filter-icon-1' }
							aria-label={ __( 'Filter icon 1', 'poocommerce' ) }
							label={ <Icon size={ 32 } icon={ filter } /> }
						/>
						<ToggleGroupControlOption
							value={ 'filter-icon-2' }
							aria-label={ __( 'Filter icon 2', 'poocommerce' ) }
							label={
						...
						<ToggleGroupControlOption
							value={ 'filter-icon-3' }
							aria-label={ __( 'Filter icon 3', 'poocommerce' ) }
							label={ <Icon size={ 32 } icon={ menu } /> }
						/>
						<ToggleGroupControlOption
							value={ 'filter-icon-4' }
							aria-label={ __( 'Filter icon 4', 'poocommerce' ) }
				...
				{ overlayButtonType !== 'label-only' && (
					<RangeControl
						...
						label={ __( 'Icon Size', 'poocommerce' ) }
