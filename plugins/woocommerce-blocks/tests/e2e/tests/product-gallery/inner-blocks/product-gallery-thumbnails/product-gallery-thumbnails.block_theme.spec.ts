	...
	test.describe( 'settings', () => {
		test( 'hides thumbnails', async ( { page, editor } ) => {
			...
			await test.step( 'in frontend', async () => {
				...
				await expect(
					productGalleryBlock.locator(
						'[data-block-name="poocommerce/product-gallery-large-image"]:visible'
					)
				).toBeVisible();
		...
		for ( const position of [ 'left', 'bottom', 'right' ] ) {
			test( `positions thumbnails to the ${ position }`, async ( {
				...
				await test.step( 'in editor', async () => {
					...
					await expect(
						productGalleryBlock.locator(
							`[data-type="poocommerce/product-gallery-thumbnails"]:${ layoutClass }(
								[data-type="poocommerce/product-gallery-large-image"]
							)`
						)
					).toBeVisible();
				...
				await test.step( 'in frontend', async () => {
					...
					await expect(
						productGalleryBlock.locator(
							`[data-block-name="poocommerce/product-gallery-thumbnails"]:${ layoutClass }(
								[data-block-name="poocommerce/product-gallery-large-image"]
							)`
						)
					).toBeVisible();
