...
class Totals extends AbstractOrderConfirmationBlock {
	...
	protected function render_content( $order, $permission = false, $attributes = [], $content = '' ) {
		...
		return $this->get_hook_content( 'poocommerce_order_details_before_order_table', [ $order ] ) . '
			<table cellspacing="0" class="wc-block-order-confirmation-totals__table ' . esc_attr( $classes_and_styles['classes'] ) . '" style="' . esc_attr( $classes_and_styles['styles'] ) . '">
				<thead>
					<tr>
						<th class="wc-block-order-confirmation-totals__product">' . esc_html__( 'Product', 'poocommerce' ) . '</th>
						<th class="wc-block-order-confirmation-totals__total">' . esc_html__( 'Total', 'poocommerce' ) . '</th>
					</tr>
				</thead>
				<tbody>
					' . $this->get_hook_content( 'poocommerce_order_details_before_order_table_items', [ $order ] ) . '
					' . $this->render_order_details_table_items( $order ) . '
					' . $this->get_hook_content( 'poocommerce_order_details_after_order_table_items', [ $order ] ) . '
				</tbody>
				<tfoot>
			...
			' . $this->get_hook_content( 'poocommerce_order_details_after_order_table', [ $order ] ) . '
			' . $this->get_hook_content( 'poocommerce_after_order_details', [ $order ] ) . '
		';
	}
	...
	protected function render_order_details_table_items( $order ) {
		...
		// phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment
			$order->get_items( apply_filters( 'poocommerce_purchase_order_item_types', 'line_item' ) ),
			function( $item ) {
                // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment
				return apply_filters( 'poocommerce_order_item_visible', true, $item );
			}
		);
	...
	protected function render_order_details_table_item( $order, $item_id, $item, $product ) {
		...
		// phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment
		$row_class = apply_filters( 'poocommerce_order_item_class', 'poocommerce-table__line-item order_item', $item, $order );
		// phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment
		...
		$item_qty = apply_filters(
			'poocommerce_order_item_quantity_html',
		...
		return '
			<tr class="' . esc_attr( $row_class ) . '">
				<td class="wc-block-order-confirmation-totals__product">
					...
					' . $this->get_hook_content( 'poocommerce_order_item_meta_start', [ $item_id, $item, $order, false ] ) . '
					' . wc_display_item_meta( $item, [ 'echo' => false ] ) . '
					' . $this->get_hook_content( 'poocommerce_order_item_meta_end', [ $item_id, $item, $order, false ] ) . '
	...
	protected function render_order_details_table_totals( $order ) {
		add_filter( 'poocommerce_order_shipping_to_display_shipped_via', '__return_empty_string' );
