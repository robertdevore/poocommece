 ...
 *          'interval' => 'week',
 *         );
 * $report = new \Automattic\PooCommerce\Admin\API\Reports\Revenue\Query( $args );
 * $mydata = $report->get_data();
 */

namespace Automattic\PooCommerce\Admin\API\Reports\Revenue;
 ...
 *  - `poocommerce_analytics_revenue_*` filters
...
class Query extends \WC_Object_Query {
	...
	public function get_data() {
		$args = apply_filters( 'poocommerce_analytics_revenue_query_args', $this->get_query_vars() );

		$data_store = \WC_Data_Store::load( 'report-revenue-stats' );
		$results    = $data_store->get_data( $args );
		return apply_filters( 'poocommerce_analytics_revenue_select_query', $results, $args );
	}
}
