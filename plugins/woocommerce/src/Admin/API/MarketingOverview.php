...
class MarketingOverview extends \WC_REST_Data_Controller {
	...
	public function activate_plugin( $request ) {
		...
		if ( ! PluginsHelper::is_plugin_installed( $plugin_slug ) ) {
			return new \WP_Error( 'poocommerce_rest_invalid_plugin', __( 'Invalid plugin.', 'poocommerce' ), 404 );
		}

		$result = activate_plugin( PluginsHelper::get_plugin_path_from_slug( $plugin_slug ) );

		if ( ! is_null( $result ) ) {
			return new \WP_Error( 'poocommerce_rest_invalid_plugin', __( 'The plugin could not be activated.', 'poocommerce' ), 500 );
		}
