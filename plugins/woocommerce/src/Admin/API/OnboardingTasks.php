...
use Automattic\PooCommerce\Internal\Admin\Onboarding\OnboardingIndustries;
...
class OnboardingTasks extends \WC_REST_Data_Controller {
	...
	public function register_routes() {
					...
					'args'                => array_merge(
						...
						array(
							'template_name' => array(
								'required'    => true,
								'type'        => 'string',
								'description' => __( 'Product template name.', 'poocommerce' ),
							),
						)
					),
				),
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'ids' => array(
							'description'       => __( 'Optional parameter to get only specific task lists by id.', 'poocommerce' ),
							'type'              => 'array',
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					'duration'     => array(
						'description'       => __( 'Time period to snooze the task.', 'poocommerce' ),
						'type'              => 'string',
					...
					'task_list_id' => array(
						'description' => __( 'Optional parameter to query specific task list.', 'poocommerce' ),
						'type'        => 'string',
					),
				),
				array(
	...
	public function create_pages_permission_check( $request ) {
		if ( ! wc_rest_check_post_permissions( 'page', 'create' ) || ! current_user_can( 'manage_options' ) ) {
			return new \WP_Error( 'poocommerce_rest_cannot_create', __( 'Sorry, you are not allowed to create new pages.', 'poocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_tasks_permission_check( $request ) {
		if ( ! current_user_can( 'manage_poocommerce' ) ) {
			return new \WP_Error( 'poocommerce_rest_cannot_create', __( 'Sorry, you are not allowed to retrieve onboarding tasks.', 'poocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function hide_task_list_permission_check( $request ) {
		if ( ! current_user_can( 'manage_poocommerce' ) ) {
			return new \WP_Error( 'poocommerce_rest_cannot_update', __( 'Sorry, you are not allowed to hide task lists.', 'poocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function snooze_task_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'manage_poocommerce' ) ) {
			return new \WP_Error( 'poocommerce_rest_cannot_create', __( 'Sorry, you are not allowed to snooze onboarding tasks.', 'poocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public static function import_sample_products_from_csv( $csv_file ) {
		...
		if ( file_exists( $csv_file ) && class_exists( 'WC_Product_CSV_Importer' ) ) {
			// Override locale so we can return mappings from PooCommerce in English language stores.
			...
			return new \WP_Error( 'poocommerce_rest_import_error', __( 'Sorry, the sample products data file was not found.', 'poocommerce' ) );
		}
	}

	/**
	 * Import sample products from PooCommerce sample CSV.
	 *
	 * @internal
	 * @return WP_Error|WP_REST_Response
	 */
	...
	public static function create_product_from_template( $request ) {
		...
		$template_path = apply_filters( 'poocommerce_product_template_csv_file_path', $template_path, $template_name );
		...
		if ( is_wp_error( $import ) || ! is_array( $import['imported'] ) || 0 === count( $import['imported'] ) ) {
			return new \WP_Error(
				'poocommerce_rest_product_creation_error',
				/* translators: %s is template name */
				__( 'Sorry, creating the product with template failed.', 'poocommerce' ),
				array( 'status' => 500 )
			);
		}
	...
	private static function get_homepage_cover_block( $image ) {
		...
		if ( ! empty( $image['url'] ) && ! empty( $image['id'] ) ) {
			...
			<div class="wp-block-cover" style="background-image:url(' . esc_url( $image['url'] ) . ')"><div class="wp-block-cover__inner-container"><!-- wp:paragraph {"align":"center","placeholder":"' . __( 'Write title…', 'poocommerce' ) . '","textColor":"white","fontSize":"large"} -->
			<p class="has-text-align-center has-large-font-size">' . __( 'Welcome to the store', 'poocommerce' ) . '</p>
			<!-- /wp:paragraph -->

			<!-- wp:paragraph {"align":"center","textColor":"white"} -->
			<p class="has-text-color has-text-align-center">' . __( 'Write a short welcome message here', 'poocommerce' ) . '</p>
			<!-- /wp:paragraph -->

			<!-- wp:buttons {"layout":{"type":"flex","justifyContent":"center"}} -->
			<div class="wp-block-buttons"><!-- wp:button -->
			<div class="wp-block-button"><a class="wp-block-button__link" href="' . esc_url( $shop_url ) . '">' . __( 'Go shopping', 'poocommerce' ) . '</a></div>
			<!-- /wp:button --></div>
			<!-- /wp:buttons --></div></div>
			<!-- /wp:cover -->';
		}
		...
		<div class="wp-block-cover"><div class="wp-block-cover__inner-container"><!-- wp:paragraph {"align":"center","placeholder":"' . __( 'Write title…', 'poocommerce' ) . '","textColor":"white","fontSize":"large"} -->
		<p class="has-text-color has-text-align-center has-large-font-size">' . __( 'Welcome to the store', 'poocommerce' ) . '</p>
		<!-- /wp:paragraph -->
	...
	private static function get_homepage_media_block( $image, $align = 'left' ) {
		...
		if ( ! empty( $image['url'] ) && ! empty( $image['id'] ) ) {
			...
			<div class="wp-block-media-text alignwide' . $css_class . '""><figure class="wp-block-media-text__media"><img src="' . esc_url( $image['url'] ) . '" alt="" class="wp-image-' . intval( $image['id'] ) . '"/></figure><div class="wp-block-media-text__content"><!-- wp:paragraph {"placeholder":"' . __( 'Content…', 'poocommerce' ) . '","fontSize":"large"} -->
		...
		<div class="wp-block-media-text alignwide' . $css_class . '"><figure class="wp-block-media-text__media"></figure><div class="wp-block-media-text__content"><!-- wp:paragraph {"placeholder":"' . __( 'Content…', 'poocommerce' ) . '","fontSize":"large"} -->
		<p class="has-large-font-size"></p>
		<!-- /wp:paragraph --></div></div>
		<!-- /wp:media-text -->';
	}
	...
	private static function get_homepage_template( $post_id ) {
		...
		if ( $products->publish >= 4 ) {
			...
			$template = self::get_homepage_cover_block( $image_1 ) . '
				...
				<h2 style="text-align:center">' . __( 'Shop by Category', 'poocommerce' ) . '</h2>
				<!-- /wp:heading -->
				<!-- wp:shortcode -->
				[product_categories number="0" parent="0"]
				<!-- /wp:shortcode -->
				<!-- wp:heading {"align":"center"} -->
				<h2 style="text-align:center">' . __( 'New In', 'poocommerce' ) . '</h2>
				<!-- /wp:heading -->
				<!-- wp:poocommerce/product-new {"columns":4} /-->
				<!-- wp:heading {"align":"center"} -->
				<h2 style="text-align:center">' . __( 'Fan Favorites', 'poocommerce' ) . '</h2>
				<!-- /wp:heading -->
				<!-- wp:poocommerce/product-top-rated {"columns":4} /-->
				<!-- wp:heading {"align":"center"} -->
				<h2 style="text-align:center">' . __( 'On Sale', 'poocommerce' ) . '</h2>
				<!-- /wp:heading -->
				<!-- wp:poocommerce/product-on-sale {"columns":4} /-->
				<!-- wp:heading {"align":"center"} -->
				<h2 style="text-align:center">' . __( 'Best Sellers', 'poocommerce' ) . '</h2>
				<!-- /wp:heading -->
			 ...
			 */
			return apply_filters( 'poocommerce_admin_onboarding_homepage_template', $template );
		}
		...
		$template = self::get_homepage_cover_block( $image_1 ) . '
		<!-- wp:heading {"align":"center"} -->
		<h2 style="text-align:center">' . __( 'New Products', 'poocommerce' ) . '</h2>
		<!-- /wp:heading -->
	...
	private static function get_available_homepage_images() {
		...
		foreach ( $industries as $industry_slug => $label ) {
			$industry_images[ $industry_slug ] = apply_filters( 'poocommerce_admin_onboarding_industry_image', WC_ADMIN_IMAGES_FOLDER_URL . '/onboarding/other-small.jpg', $industry_slug );
		}
	...
	public static function create_homepage() {
		$post_id = wp_insert_post(
			array(
				'post_title'   => __( 'Homepage', 'poocommerce' ),
				'post_type'    => 'page',
				'post_status'  => 'publish',
		...
		if ( ! is_wp_error( $post_id ) && 0 < $post_id ) {
			...
			update_option( 'poocommerce_onboarding_homepage_post_id', $post_id );
			...
			return array(
				'status'         => 'success',
				'message'        => __( 'Homepage created', 'poocommerce' ),
				'post_id'        => $post_id,
	...
	public function get_task_list_params() {
		...
		$params['extended_tasks'] = array(
			'description'       => __( 'List of extended deprecated tasks from the client side filter.', 'poocommerce' ),
			'type'              => 'array',
			'validate_callback' => function( $param, $request, $key ) {
				$has_valid_keys = true;
	...
	public function get_tasks( $request ) {
		...
		return rest_ensure_response( array_values( apply_filters( 'poocommerce_admin_onboarding_tasks', $json ) ) );
	}
	...
	public function dismiss_task( $request ) {
		...
		if ( ! $task || ! $task->is_dismissable() ) {
			return new \WP_Error(
				'poocommerce_rest_invalid_task',
				__( 'Sorry, no dismissable task with that ID was found.', 'poocommerce' ),
				array(
					'status' => 404,
				)
			);
	...
	public function snooze_task( $request ) {
		...
		if ( ! $task || ! $task->is_snoozeable() ) {
			return new \WP_Error(
				'poocommerce_rest_invalid_task',
				__( 'Sorry, no snoozeable task with that ID was found.', 'poocommerce' ),
				array(
					'status' => 404,
				)
			);
	...
	public function hide_task_list( $request ) {
		...
		if ( ! $task_list ) {
			return new \WP_Error(
				'poocommerce_rest_invalid_task_list',
				__( 'Sorry, that task list was not found', 'poocommerce' ),
				array(
					'status' => 404,
				)
			);
	...
	public function unhide_task_list( $request ) {
		...
		if ( ! $task_list ) {
			return new \WP_Error(
				'poocommerce_tasks_invalid_task_list',
				__( 'Sorry, that task list was not found', 'poocommerce' ),
				array(
					'status' => 404,
				)
	...
	public function action_task( $request ) {
		...
		if ( ! $task ) {
			return new \WP_Error(
				'poocommerce_rest_invalid_task',
				__( 'Sorry, no task with that ID was found.', 'poocommerce' ),
				array(
					'status' => 404,
				)
			);
