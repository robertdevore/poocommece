<?php
/**
 * PooCommerce Admin Real Time Order Alerts Note.
 *
...
class RealTimeOrderAlerts {
	...
	public static function get_note() {
		...
		$content = __( 'Get notifications about store activity, including new orders and product reviews directly on your mobile devices with the Woo app.', 'poocommerce' );

		$note = new Note();
		$note->set_title( __( 'Get real-time order alerts anywhere', 'poocommerce' ) );
		$note->set_content( $content );
		$note->set_content_data( (object) array() );
		$note->set_type( Note::E_WC_ADMIN_NOTE_INFORMATIONAL );
		$note->set_name( self::NOTE_NAME );
		$note->set_source( 'poocommerce-admin' );
		$note->add_action( 'learn-more', __( 'Learn more', 'poocommerce' ), 'https://poocommerce.com/mobile/?utm_source=inbox&utm_medium=product' );
		return $note;
	}
}
