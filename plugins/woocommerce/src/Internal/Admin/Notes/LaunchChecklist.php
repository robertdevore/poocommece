<?php
/**
 * PooCommerce Admin Launch Checklist Note.
 *
...
class LaunchChecklist {
	...
	public static function get_note() {
		...
		if (
			! get_option( 'poocommerce_task_list_complete' ) &&
			(
		...
		$content = __( 'To make sure you never get that sinking "what did I forget" feeling, we\'ve put together the essential pre-launch checklist.', 'poocommerce' );

		$note = new Note();
		$note->set_title( __( 'Ready to launch your store?', 'poocommerce' ) );
		$note->set_content( $content );
		$note->set_content_data( (object) array() );
		$note->set_type( Note::E_WC_ADMIN_NOTE_INFORMATIONAL );
		$note->set_name( self::NOTE_NAME );
		$note->set_source( 'poocommerce-admin' );
		$note->add_action( 'learn-more', __( 'Learn more', 'poocommerce' ), 'https://poocommerce.com/posts/pre-launch-checklist-the-essentials/?utm_source=inbox&utm_medium=product' );
		return $note;
	}
}
