...
class McStats extends A8c_Mc_Stats {
	 ...
	 *
	 * Jetpack MC Stats package prefixes group names with "x_jetpack-" so we override this method to prefix group names with "x_poocommerce-".
	 *
	...
	public function get_group_query_args( $group_name ) {
		...
		if ( isset( $stats[ $group_name ] ) && ! empty( $stats[ $group_name ] ) ) {
			return array( "x_poocommerce-{$group_name}" => implode( ',', $stats[ $group_name ] ) );
		}
		return array();
	}
	 ...
	 *
	 * @param string $url string The URL to be pinged. Should include `x_poocommerce-{$group}={$stats}` or whatever we want to store.
	 *
