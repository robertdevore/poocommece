...
class OrdersTableQuery {
	...
	private function date_to_date_query_arg( $date ): array {
		...
		if ( is_numeric( $date ) ) {
			...
			// For backwards compat (see https://github.com/poocommerce/poocommerce/wiki/wc_get_orders-and-WC_Order_Query#date)
	...
	private function local_time_to_gmt_date_query( $dates_raw, $operator ) {
		...
		// Convert YYYY-MM-DD to UTC timestamp. Per https://github.com/poocommerce/poocommerce/wiki/wc_get_orders-and-WC_Order_Query#date only date is relevant (time is ignored).
		...
		foreach ( $date_keys as $date_key ) {
			...
			if ( $is_local ) {
				...
				if ( ! is_numeric( $dates_raw[0] ) && ( ! isset( $dates_raw[1] ) || ! is_numeric( $dates_raw[1] ) ) ) {
					// Only non-numeric args can be considered local time. Timestamps are assumed to be UTC per https://github.com/poocommerce/poocommerce/wiki/wc_get_orders-and-WC_Order_Query#date.
