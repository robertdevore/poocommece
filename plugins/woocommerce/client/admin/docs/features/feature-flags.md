...
Features inside the `poocommerce` repository can be in various states of completeness. To provide a way for improved control over how these features are released in these different environments, `poocommerce` has a system for feature flags.
...
```
<body class="wp-admin poocommerce-admin-page poocommerce-feature-enabled-activity-panels  ....">
```
...
To check if a feature is enabled, you can use the `Automattic\PooCommerce\Admin\Features\Features::is_enabled()`:

```
if ( \Automattic\PooCommerce\Admin\Features\Features::is_enabled( 'activity-panels' ) ) {
