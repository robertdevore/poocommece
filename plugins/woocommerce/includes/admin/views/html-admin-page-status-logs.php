	...
	<div id="log-viewer-select">
		<div class="alignleft">
			<h2>
				...
				<?php if ( ! empty( $viewed_log ) ) : ?>
					<a class="page-title-action" href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'handle' => sanitize_title( $viewed_log ) ), admin_url( 'admin.php?page=wc-status&tab=logs' ) ), 'remove_log' ) ); ?>" class="button"><?php esc_html_e( 'Delete log', 'poocommerce' ); ?></a>
				<?php endif; ?>
			</h2>
		</div>
		<div class="alignright">
			<form action="<?php echo esc_url( admin_url( 'admin.php?page=wc-status&tab=logs' ) ); ?>" method="post">
				...
				<button type="submit" class="button" value="<?php esc_attr_e( 'View', 'poocommerce' ); ?>"><?php esc_html_e( 'View', 'poocommerce' ); ?></button>
			</form>
		</div>
		<div class="clear"></div>
	</div>
...
<?php else : ?>
	<div class="updated poocommerce-message inline"><p><?php esc_html_e( 'There are currently no logs to view.', 'poocommerce' ); ?></p></div>
<?php endif; ?>
