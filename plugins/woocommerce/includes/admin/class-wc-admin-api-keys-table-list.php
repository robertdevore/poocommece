<?php
/**
 * PooCommerce API Keys Table List
 *
...
class WC_Admin_API_Keys_Table_List extends WP_List_Table {
	...
	public function no_items() {
		esc_html_e( 'No keys found.', 'poocommerce' );
	}
	...
	public function get_columns() {
		return array(
			...
			'truncated_key' => __( 'Consumer key ending in', 'poocommerce' ),
			'user'          => __( 'User', 'poocommerce' ),
			'permissions'   => __( 'Permissions', 'poocommerce' ),
			'last_access'   => __( 'Last access', 'poocommerce' ),
		);
	}
	...
	public function column_title( $key ) {
		...
		if ( empty( $key['description'] ) ) {
			$output .= esc_html__( 'API key', 'poocommerce' );
		} else {
		...
		$actions = array(
			...
			'id' => sprintf( __( 'ID: %d', 'poocommerce' ), $key['key_id'] ),
		);

		if ( $can_edit ) {
			$actions['edit']  = '<a href="' . esc_url( $url ) . '">' . __( 'View/Edit', 'poocommerce' ) . '</a>';
			$actions['trash'] = '<a class="submitdelete" aria-label="' . esc_attr__( 'Revoke API key', 'poocommerce' ) . '" href="' . esc_url(
				wp_nonce_url(
					add_query_arg(
						array(
			...
			) . '">' . esc_html__( 'Revoke', 'poocommerce' ) . '</a>';
		}
	...
	public function column_last_access( $key ) {
		if ( ! empty( $key['last_access'] ) ) {
			...
			$date = sprintf( __( '%1$s at %2$s', 'poocommerce' ), date_i18n( wc_date_format(), strtotime( $key['last_access'] ) ), date_i18n( wc_time_format(), strtotime( $key['last_access'] ) ) );

			return apply_filters( 'poocommerce_api_key_last_access_datetime', $date, $key['last_access'] );
		}

		return __( 'Unknown', 'poocommerce' );
	}
	...
	public function prepare_items() {
		...
		$per_page     = $this->get_items_per_page( 'poocommerce_keys_per_page' );
		...
		$keys = $wpdb->get_results(
			"SELECT key_id, user_id, description, permissions, truncated_key, last_access FROM {$wpdb->prefix}poocommerce_api_keys WHERE 1 = 1 {$search}" .
			$wpdb->prepare( 'ORDER BY key_id DESC LIMIT %d OFFSET %d;', $per_page, $offset ),
			ARRAY_A
		); // WPCS: unprepared SQL ok.

		$count = $wpdb->get_var( "SELECT COUNT(key_id) FROM {$wpdb->prefix}poocommerce_api_keys WHERE 1 = 1 {$search};" ); // WPCS: unprepared SQL ok.
