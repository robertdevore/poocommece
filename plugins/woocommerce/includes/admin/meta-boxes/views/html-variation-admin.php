...
?>
<div class="poocommerce_variation wc-metabox closed">
	<h3>
		<a href="javascript:void(0)" class="edit_variation edit"><?php esc_html_e( 'Edit', 'poocommerce' ); ?></a>
		<a href="#" class="remove_variation delete" rel="<?php echo esc_attr( $variation_id ); ?>"><?php esc_html_e( 'Remove', 'poocommerce' ); ?></a>
		<div class="tips sort" data-tip="<?php esc_attr_e( 'Drag and drop, or click to set admin variation order', 'poocommerce' ); ?>"></div>
		...
		foreach ( $product_object->get_attributes( 'edit' ) as $attribute ) {
			...
			<select name="attribute_<?php echo esc_attr( sanitize_title( $attribute->get_name() ) . "[{$loop}]" ); ?>">
				<option value="">
					...
					printf( esc_html__( 'Any %s&hellip;', 'poocommerce' ), wc_attribute_label( $attribute->get_name() ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					?>
				</option>
		 ...
		 */
		do_action( 'poocommerce_variation_header', $variation, $loop );
		?>
	</h3>
	<div class="poocommerce_variable_attributes wc-metabox-content" style="display: none;">
		<div class="data">
			<div class="form-flex-box">
				<p class="form-row upload_image">
					<a href="#" class="upload_image_button tips <?php echo $variation_object->get_image_id( 'edit' ) ? 'remove' : ''; ?>" data-tip="<?php echo $variation_object->get_image_id( 'edit' ) ? esc_attr__( 'Remove this image', 'poocommerce' ) : esc_attr__( 'Upload an image', 'poocommerce' ); ?>" rel="<?php echo esc_attr( $variation_id ); ?>">
			...
			<p class="form-row form-row-full options">
				<label>
					<?php esc_html_e( 'Enabled', 'poocommerce' ); ?>
				...
				<label class="tips" data-tip="<?php esc_attr_e( 'Enable this option if access is given to a downloadable file upon purchase of a product', 'poocommerce' ); ?>">
					<?php esc_html_e( 'Downloadable', 'poocommerce' ); ?>
				...
				<label class="tips" data-tip="<?php esc_attr_e( 'Enable this option if a product is not shipped or there is no shipping cost', 'poocommerce' ); ?>">
					<?php esc_html_e( 'Virtual', 'poocommerce' ); ?>
				...
				<?php if ( 'yes' === get_option( 'poocommerce_manage_stock' ) ) : ?>
					<label class="tips" data-tip="<?php esc_attr_e( 'Enable this option to enable stock management at variation level', 'poocommerce' ); ?>">
						<?php esc_html_e( 'Manage stock?', 'poocommerce' ); ?>
				...
				<?php /* phpcs:disable PooCommerce.Commenting.CommentHooks.MissingHookComment */ ?>
				<?php do_action( 'poocommerce_variation_options', $loop, $variation_data, $variation ); ?>
				<?php /* phpcs:enable */ ?>
			</p>
			...
			<div class="variable_pricing">
				...
				$label = sprintf(
					...
					__( 'Regular price (%s)', 'poocommerce' ),
				...
				poocommerce_wp_text_input(
					array(
						...
						'placeholder'   => __( 'Variation price (required)', 'poocommerce' ),
					)
				);

				$label = sprintf(
					/* translators: %s: currency symbol */
					__( 'Sale price (%s)', 'poocommerce' ),
					get_poocommerce_currency_symbol()
				);
				...
				poocommerce_wp_text_input(
					array(
						...
						'data_type'     => 'price',
						'label'         => $label . ' <a href="#" class="sale_schedule">' . esc_html__( 'Schedule', 'poocommerce' ) . '</a><a href="#" class="cancel_sale_schedule hidden">' . esc_html__( 'Cancel schedule', 'poocommerce' ) . '</a>',
				...
				echo '<div class="form-field sale_price_dates_fields hidden">
					<p class="form-row form-row-first">
						<label>' . esc_html__( 'Sale start date', 'poocommerce' ) . '</label>
						<input type="text" class="sale_price_dates_from" name="variable_sale_price_dates_from[' . esc_attr( $loop ) . ']" value="' . esc_attr( $sale_price_dates_from ) . '" placeholder="' . esc_attr_x( 'From&hellip;', 'placeholder', 'poocommerce' ) . ' YYYY-MM-DD" maxlength="10" pattern="' . esc_attr( apply_filters( 'poocommerce_date_input_html_pattern', '[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])' ) ) . '" />
					</p>
					<p class="form-row form-row-last">
						<label>' . esc_html__( 'Sale end date', 'poocommerce' ) . '</label>
				 ...
				 * @param WP_Post $variation      Post data.
				 */
				do_action( 'poocommerce_variation_options_pricing', $loop, $variation_data, $variation );
				?>
			</div>
				...
				<div class="show_if_variation_manage_stock" style="display: none;">
					...
					poocommerce_wp_text_input(
						array(
							...
							'desc_tip'          => true,
							'description'       => __( "Enter a number to set stock quantity at the variation level. Use a variation's 'Manage stock?' check box above to enable/disable stock management at the variation level.", 'poocommerce' ),
							'type'              => 'number',
					...
					poocommerce_wp_select(
						array(
							...
							'desc_tip'      => true,
							'description'   => __( 'If managing stock, this controls whether or not backorders are allowed. If enabled, stock quantity can go below 0.', 'poocommerce' ),
							'wrapper_class' => 'form-row form-row-last',
						)
					);
					...
					$low_stock_placeholder = ( $product_object->get_manage_stock() && '' !== $product_object->get_low_stock_amount() )
						? sprintf(
							...
							esc_attr__( 'Parent product\'s threshold (%d)', 'poocommerce' ),
					...
					poocommerce_wp_text_input(
						array(
							...
							'desc_tip'          => true,
							'description'       => __( 'When variation stock reaches this amount you will be notified by email. The default value for all variations can be set in the product Inventory tab. The shop default value can be set in Settings > Products > Inventory.', 'poocommerce' ),
							'type'              => 'number',
							'custom_attributes' => array(
								'step' => 'any',
							),
					 ...
					 * @param WP_Post $variation      Post data.
					 */
					do_action( 'poocommerce_variation_options_inventory', $loop, $variation_data, $variation );
					?>
				</div>
			<?php endif; ?>
			...
			<div>
					...
					<p class="form-field form-row dimensions_field hide_if_variation_virtual form-row-last">
						<label for="product_length">
							...
							printf(
								...
								esc_html__( 'Dimensions (L&times;W&times;H) (%s)', 'poocommerce' ),
								esc_html( I18nUtil::get_dimensions_unit_label( get_option( 'poocommerce_dimension_unit' ) ) )
							);
							?>
						</label>
						<?php echo wc_help_tip( __( 'Length x width x height in decimal form', 'poocommerce' ) ); ?>
						<span class="wrap">
							<input id="product_length" placeholder="<?php echo $parent_length ? esc_attr( $parent_length ) : esc_attr__( 'Length', 'poocommerce' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="variable_length[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_length( 'edit' ) ) ); ?>" />
							<input placeholder="<?php echo $parent_width ? esc_attr( $parent_width ) : esc_attr__( 'Width', 'poocommerce' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="variable_width[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_width( 'edit' ) ) ); ?>" />
							<input placeholder="<?php echo $parent_height ? esc_attr( $parent_height ) : esc_attr__( 'Height', 'poocommerce' ); ?>" class="input-text wc_input_decimal last" size="6" type="text" name="variable_height[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_height( 'edit' ) ) ); ?>" />
						</span>
					</p>
					<?php
				}
				 ...
				 * @param WP_Post $variation      Post data.
				 */
				do_action( 'poocommerce_variation_options_dimensions', $loop, $variation_data, $variation );
				?>
			</div>

			<div>
				<p class="form-row hide_if_variation_virtual form-row-full">
					<label><?php esc_html_e( 'Shipping class', 'poocommerce' ); ?></label>
					<?php
					wp_dropdown_categories(
						array(
							'taxonomy'         => 'product_shipping_class',
							'hide_empty'       => 0,
							'show_option_none' => __( 'Same as parent', 'poocommerce' ),
				...
				if ( wc_tax_enabled() ) {
					poocommerce_wp_select(
						array(
							...
							'options'       => array( 'parent' => __( 'Same as parent', 'poocommerce' ) ) + wc_get_product_tax_class_options(),
							'desc_tip'      => 'true',
					 ...
					 * @param WP_Post $variation      Post data.
					 */
					do_action( 'poocommerce_variation_options_tax', $loop, $variation_data, $variation );
				}
				?>
			</div>
			<div>
				...
				poocommerce_wp_textarea_input(
					array(
						...
						'desc_tip'      => true,
						'description'   => __( 'Enter an optional description for this variation.', 'poocommerce' ),
						'wrapper_class' => 'form-row form-row-full',
					)
				);
				?>
			...
			<div class="show_if_variation_downloadable" style="display: none;">
				 ...
				 * @param WP_Post $variation      Post data.
				 */
				do_action( 'poocommerce_variation_options_download', $loop, $variation_data, $variation );
				?>
			</div>
			<?php /* phpcs:disable PooCommerce.Commenting.CommentHooks.MissingHookComment */ ?>
			<?php do_action( 'poocommerce_product_after_variable_attributes', $loop, $variation_data, $variation ); ?>
			<?php /* phpcs:enable */ ?>
		</div>
	</div>
</div>
