	...
	<td class="name">
		...
		if ( $who_refunded->exists() ) {
			printf(
				...
				esc_html__( 'Refund #%1$s - %2$s by %3$s', 'poocommerce' ),
				...
				sprintf(
					...
					sprintf( esc_attr__( 'ID: %d', 'poocommerce' ), absint( $who_refunded->ID ) ),
			...
			printf(
				...
				esc_html__( 'Refund #%1$s - %2$s', 'poocommerce' ),
				esc_html( $refund->get_id() ),
				esc_html( wc_format_datetime( $refund->get_date_created(), get_option( 'date_format' ) . ', ' . get_option( 'time_format' ) ) )
			);
		}
		...
		<?php do_action( 'poocommerce_after_order_refund_item_name', $refund ); ?>
	</td>

	<?php do_action( 'poocommerce_admin_order_item_values', null, $refund, $refund->get_id() ); ?>
