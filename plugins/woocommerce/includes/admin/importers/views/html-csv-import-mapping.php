...
?>
<form class="wc-progress-form-content poocommerce-importer" method="post" action="<?php echo esc_url( $this->get_next_step_link() ); ?>">
	<header>
		<h2><?php esc_html_e( 'Map CSV fields to products', 'poocommerce' ); ?></h2>
		<p><?php esc_html_e( 'Select fields from your CSV file to map against products fields, or to ignore during import.', 'poocommerce' ); ?></p>
	</header>
	<section class="wc-importer-mapping-table-wrapper">
		<table class="widefat wc-importer-mapping-table">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Column name', 'poocommerce' ); ?></th>
					<th><?php esc_html_e( 'Map to field', 'poocommerce' ); ?></th>
				</tr>
			</thead>
			<tbody>
					...
					<tr>
						<td class="wc-importer-mapping-table-name">
							...
							<?php if ( ! empty( $sample[ $index ] ) ) : ?>
								<span class="description"><?php esc_html_e( 'Sample:', 'poocommerce' ); ?> <code><?php echo esc_html( $sample[ $index ] ); ?></code></span>
							<?php endif; ?>
						</td>
						<td class="wc-importer-mapping-table-field">
							...
							<select name="map_to[<?php echo esc_attr( $index ); ?>]">
								<option value=""><?php esc_html_e( 'Do not import', 'poocommerce' ); ?></option>
								<option value="">--------------</option>
	...
	<div class="wc-actions">
		<button type="submit" class="button button-primary button-next" value="<?php esc_attr_e( 'Run the importer', 'poocommerce' ); ?>" name="save_step"><?php esc_html_e( 'Run the importer', 'poocommerce' ); ?></button>
