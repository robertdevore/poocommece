...
class WC_Marketplace_Updater {
	...
	public static function init() {
		add_action( 'poocommerce_update_marketplace_suggestions', array( __CLASS__, 'update_marketplace_suggestions' ) );
	}
	...
	public static function update_marketplace_suggestions() {
		$data = get_option(
			'poocommerce_marketplace_suggestions',
			array(
				'suggestions' => array(),
				'updated'     => time(),
			)
		...
		$url     = 'https://poocommerce.com/wp-json/wccom/marketplace-suggestions/1.0/suggestions.json';
		...
		if ( is_wp_error( $request ) ) {
			self::retry();
			return update_option( 'poocommerce_marketplace_suggestions', $data, false );
		}
	...
	public static function retry() {
		...
		WC()->queue()->schedule_single( time() + DAY_IN_SECONDS, 'poocommerce_update_marketplace_suggestions' );
	}
}
