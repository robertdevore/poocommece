...
function wc_load_persistent_cart( $user_login, $user ) {
	if ( ! $user || ! apply_filters( 'poocommerce_persistent_cart_enabled', true ) ) {
		return;
	}

	$saved_cart = get_user_meta( $user->ID, '_poocommerce_persistent_cart_' . get_current_blog_id(), true );
...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	foreach ( $products as $product_id => $qty ) {
		...
		$titles[] = apply_filters( 'poocommerce_add_to_cart_qty_html', ( $qty > 1 ? absint( $qty ) . ' &times; ' : '' ), $product_id ) . apply_filters( 'poocommerce_add_to_cart_item_name_in_quotes', sprintf( _x( '&ldquo;%s&rdquo;', 'Item name in quotes', 'poocommerce' ), strip_tags( get_the_title( $product_id ) ) ), $product_id );
		$count   += $qty;
	}
	...
	foreach ( $packages as $i => $package ) {
		...
		wc_get_template(
			...
			array(
				...
				'package_name'             => apply_filters( 'poocommerce_shipping_package_name', ( ( $i + 1 ) > 1 ) ? sprintf( _x( 'Shipping %d', 'shipping packages', 'poocommerce' ), ( $i + 1 ) ) : _x( 'Shipping', 'shipping packages', 'poocommerce' ), $i, $package ),
				'index'                    => $i,
...
function wc_cart_totals_coupon_html( $coupon ) {
	...
	$coupon_html          = $discount_amount_html . ' <a href="' . esc_url( add_query_arg( 'remove_coupon', rawurlencode( $coupon->get_code() ), Constants::is_defined( 'WOOCOMMERCE_CHECKOUT' ) ? wc_get_checkout_url() : wc_get_cart_url() ) ) . '" class="poocommerce-remove-coupon" data-coupon="' . esc_attr( $coupon->get_code() ) . '">' . __( '[Remove]', 'poocommerce' ) . '</a>';
