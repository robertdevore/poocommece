...
class WC_Discounts {
	...
	protected function validate_coupon_exists( $coupon ) {
		if ( ( ! $coupon->get_id() && ! $coupon->get_virtual() ) || 'trash' === $coupon->get_status() ) {
			...
			throw new Exception( sprintf( __( 'Coupon "%s" does not exist!', 'poocommerce' ), esc_html( $coupon->get_code() ) ), 105 );
		}
	...
	protected function validate_coupon_expiry_date( $coupon ) {
		if ( $coupon->get_date_expires() && apply_filters( 'poocommerce_coupon_validate_expiry_date', time() > $coupon->get_date_expires()->getTimestamp(), $coupon, $this ) ) {
			throw new Exception( __( 'This coupon has expired.', 'poocommerce' ), 107 );
		}
	...
	protected function validate_coupon_minimum_amount( $coupon ) {
		...
		if ( $coupon->get_minimum_amount() > 0 && apply_filters( 'poocommerce_coupon_validate_minimum_amount', $coupon->get_minimum_amount() > $subtotal, $coupon, $subtotal ) ) {
			...
			throw new Exception( sprintf( __( 'The minimum spend for this coupon is %s.', 'poocommerce' ), wc_price( $coupon->get_minimum_amount() ) ), 108 );
		}
	...
	protected function validate_coupon_maximum_amount( $coupon ) {
		...
		if ( $coupon->get_maximum_amount() > 0 && apply_filters( 'poocommerce_coupon_validate_maximum_amount', $coupon->get_maximum_amount() < $subtotal, $coupon ) ) {
			...
			throw new Exception( sprintf( __( 'The maximum spend for this coupon is %s.', 'poocommerce' ), wc_price( $coupon->get_maximum_amount() ) ), 112 );
		}
	...
	protected function validate_coupon_product_ids( $coupon ) {
		if ( count( $coupon->get_product_ids() ) > 0 ) {
			...
			if ( ! $valid ) {
				throw new Exception( __( 'Sorry, this coupon is not applicable to selected products.', 'poocommerce' ), 109 );
			}
		}
	...
	protected function validate_coupon_sale_items( $coupon ) {
		if ( $coupon->get_exclude_sale_items() ) {
			...
			if ( ! $valid ) {
				throw new Exception( __( 'Sorry, this coupon is not valid for sale items.', 'poocommerce' ), 110 );
			}
		}
	...
	protected function validate_coupon_excluded_product_ids( $coupon ) {
		...
		if ( count( $coupon->get_excluded_product_ids() ) > 0 ) {
			...
			if ( ! empty( $products ) ) {
				...
				throw new Exception( sprintf( __( 'Sorry, this coupon is not applicable to the products: %s.', 'poocommerce' ), implode( ', ', $products ) ), 113 );
			}
		}
	...
	protected function validate_coupon_excluded_product_categories( $coupon ) {
		if ( count( $coupon->get_excluded_product_categories() ) > 0 ) {
			...
			if ( ! empty( $categories ) ) {
				...
				throw new Exception( sprintf( __( 'Sorry, this coupon is not applicable to the categories: %s.', 'poocommerce' ), implode( ', ', array_unique( $categories ) ) ), 114 );
			}
		}
