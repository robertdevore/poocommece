...
class WC_Queue {
	...
	protected static function get_class() {
		if ( ! did_action( 'plugins_loaded' ) ) {
			wc_doing_it_wrong( __FUNCTION__, __( 'This function should not be called before plugins_loaded.', 'poocommerce' ), '3.5.0' );
		}

		return apply_filters( 'poocommerce_queue_class', self::$default_cass );
	}
	...
	protected static function validate_instance( $instance ) {
		if ( false === ( $instance instanceof WC_Queue_Interface ) ) {
			...
			wc_doing_it_wrong( __FUNCTION__, sprintf( __( 'The class attached to the "poocommerce_queue_class" does not implement the WC_Queue_Interface interface. The default %s class will be used instead.', 'poocommerce' ), $default_class ), '3.5.0' );
