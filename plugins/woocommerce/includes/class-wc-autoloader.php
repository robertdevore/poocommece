...
class WC_Autoloader {
	...
	public function autoload( $class ) {
		...
		// The Legacy REST API was removed in PooCommerce 9.0, but some servers still have
		...
		// when it's provided by the PooCommerce Legacy REST API plugin.
		if ( 'wc_api' === $class ) {
			return;
		}

		// If the class is already loaded from a merged package, prevent autoloader from loading it as well.
		if ( \Automattic\PooCommerce\Packages::should_load_class( $class ) ) {
			return;
		}
