...
class WC_REST_Orders_V2_Controller extends WC_REST_CRUD_Controller {
	...
	protected function prepare_objects_query( $request ) {
		...
		if ( ! empty( $request['product'] ) ) {
			$order_ids = $wpdb->get_col(
				$wpdb->prepare(
					...
					WHERE order_item_id IN ( SELECT order_item_id FROM {$wpdb->prefix}poocommerce_order_itemmeta WHERE meta_key = '_product_id' AND meta_value = %d )
		 ...
		 * @since 4.5.0.
		 */
		$args = apply_filters( 'poocommerce_rest_orders_prepare_object_query', $args, $request );
	...
	protected function get_product_id( $posted, $action = 'create' ) {
		if ( ! empty( $posted['sku'] ) ) {
			...
			throw new WC_REST_Exception( 'poocommerce_rest_required_product_reference', __( 'Product ID or SKU is required.', 'poocommerce' ), 400 );
		}
		return $product_id;
	}
	...
	protected function prepare_shipping_lines( $posted, $action = 'create', $item = null ) {
		...
		if ( 'create' === $action ) {
			if ( empty( $posted['method_id'] ) ) {
				throw new WC_REST_Exception( 'poocommerce_rest_invalid_shipping_item', __( 'Shipping method ID is required.', 'poocommerce' ), 400 );
			}
		}
	...
	protected function prepare_fee_lines( $posted, $action = 'create', $item = null ) {
		...
		if ( 'create' === $action ) {
			if ( empty( $posted['name'] ) ) {
				throw new WC_REST_Exception( 'poocommerce_rest_invalid_fee_item', __( 'Fee name is required.', 'poocommerce' ), 400 );
			}
		}
	...
	protected function prepare_coupon_lines( $posted, $action = 'create', $item = null ) {
		...
		if ( 'create' === $action ) {
			...
			if ( StringUtil::is_null_or_whitespace( $coupon_code ) ) {
				throw new WC_REST_Exception( 'poocommerce_rest_invalid_coupon_coupon', __( 'Coupon code is required.', 'poocommerce' ), 400 );
			}
		}
	...
	protected function set_item( $order, $item_type, $posted ) {
		...
		if ( 'update' === $action ) {
			...
			if ( ! $item ) {
				throw new WC_REST_Exception( 'poocommerce_rest_invalid_item_id', __( 'Order item ID provided is not associated with order.', 'poocommerce' ), 400 );
			}
		}
		 ...
		 * @since 4.5.0.
		 */
		do_action( 'poocommerce_rest_set_order_item', $item, $posted );
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'line_items'           => array(
					...
					'items'       => array(
						...
						'properties' => array(
							...
							'parent_name'  => array(
								'description' => __( 'Parent product name if the product is a variation.', 'poocommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
							),
							'product_id'   => array(
							...
							'image'        => array(
								'description' => __( 'Properties of the main product image.', 'poocommerce' ),
								'type'        => 'object',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
								'properties'  => array(
				...
				'refunds'              => array(
					'description' => __( 'List of refunds.', 'poocommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
					'items'       => array(
						...
						'properties' => array(
							'id'     => array(
								'description' => __( 'Refund ID.', 'poocommerce' ),
								'type'        => 'integer',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'reason' => array(
								'description' => __( 'Refund reason.', 'poocommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'total'  => array(
								'description' => __( 'Refund total.', 'poocommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
	...
	public function get_collection_params() {
		...
		$params['status']                  = array(
			'default'           => 'any',
			'description'       => __( 'Limit result set to orders assigned a specific status.', 'poocommerce' ),
			'type'              => 'string',
		...
		$params['customer']                = array(
			'description'       => __( 'Limit result set to orders assigned a specific customer.', 'poocommerce' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['product']                 = array(
			'description'       => __( 'Limit result set to orders assigned a specific product.', 'poocommerce' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
		...
		$params['order_item_display_meta'] = array(
			'default'           => false,
			'description'       => __( 'Only show meta which is meant to be displayed for an order.', 'poocommerce' ),
			'type'              => 'boolean',
