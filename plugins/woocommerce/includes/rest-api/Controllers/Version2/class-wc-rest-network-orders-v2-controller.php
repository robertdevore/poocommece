...
class WC_REST_Network_Orders_V2_Controller extends WC_REST_Orders_V2_Controller {
	...
	public function get_public_item_schema() {
		...
		$schema['properties']['blog']              = array(
			'description' => __( 'Blog id of the record on the multisite.', 'poocommerce' ),
			'type'        => 'integer',
			'context'     => array( 'view' ),
			'readonly'    => true,
		);
		$schema['properties']['edit_url']          = array(
			'description' => __( 'URL to edit the order', 'poocommerce' ),
			'type'        => 'string',
			'context'     => array( 'view' ),
			'readonly'    => true,
		);
		$schema['properties']['customer'][]        = array(
			'description' => __( 'Name of the customer for the order', 'poocommerce' ),
			'type'        => 'string',
			'context'     => array( 'view' ),
			'readonly'    => true,
		);
		...
		$schema['properties']['formatted_total'][] = array(
			'description' => __( 'Order total formatted for locale', 'poocommerce' ),
			'type'        => 'string',
			'context'     => array( 'view' ),
			'readonly'    => true,
		);
	...
	public function network_orders( $request ) {
		...
		foreach ( $plugins as $plugin ) {
			if ( substr_compare( $plugin, '/poocommerce.php', strlen( $plugin ) - strlen( '/poocommerce.php' ), strlen( '/poocommerce.php' ) ) === 0 ) {
				$wc_active = true;
			}
		}

		// If PooCommerce not active for site, return an empty response.
		if ( ! $wc_active ) {
		...
		switch_to_blog( $blog_id );
		add_filter( 'poocommerce_rest_orders_prepare_object_query', array( $this, 'network_orders_filter_args' ) );
		$items = $this->get_items( $request );
		remove_filter( 'poocommerce_rest_orders_prepare_object_query', array( $this, 'network_orders_filter_args' ) );
		...
		foreach ( $items->data as &$current_order ) {
			...
			$current_order['customer']        = trim( sprintf( _x( '%1$s %2$s', 'full name', 'poocommerce' ), $order->get_billing_first_name(), $order->get_billing_last_name() ) );
