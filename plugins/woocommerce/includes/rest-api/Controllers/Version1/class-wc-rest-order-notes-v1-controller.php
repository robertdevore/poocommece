...
class WC_REST_Order_Notes_V1_Controller extends WC_REST_Controller {
	...
	public function create_item( $request ) {
		...
		if ( ! $order || $this->post_type !== $order->get_type() ) {
			return new WP_Error( 'poocommerce_rest_order_invalid_id', __( 'Invalid order ID.', 'poocommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( ! $note_id ) {
			return new WP_Error( 'poocommerce_api_cannot_create_order_note', __( 'Cannot create order note, please try again.', 'poocommerce' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'poocommerce_rest_insert_order_note', $note, $request, true );
	...
	public function delete_item( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error( 'poocommerce_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'poocommerce' ), 'order_note' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'poocommerce_rest_delete_order_note', $note, $response, $request );
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'note' => array(
					'description' => __( 'Order note.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'customer_note' => array(
					'description' => __( 'Shows/define if the note is only for reference or for the customer (the user will be notified).', 'poocommerce' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'view', 'edit' ),
				),
