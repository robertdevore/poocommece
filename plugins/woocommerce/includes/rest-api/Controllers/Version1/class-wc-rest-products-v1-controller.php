...
class WC_REST_Products_V1_Controller extends WC_REST_Posts_Controller {
	...
	public function __construct() {
		...
		add_action( "poocommerce_rest_insert_{$this->post_type}", array( $this, 'clear_transients' ) );
	}
	...
	protected function get_product_data( $product ) {
		$data = array(
			...
			'short_description'     => apply_filters( 'poocommerce_short_description', $product->get_short_description() ),
			'sku'                   => $product->get_sku(),
	...
	protected function prepare_item_for_database( $request ) {
		 ...
		 * @param WP_REST_Request $request       Request object.
		 */
		return apply_filters( "poocommerce_rest_pre_insert_{$this->post_type}", $product, $request );
	}
	...
	public function create_item( $request ) {
		...
		try {
			 ...
			 */
			do_action( 'poocommerce_rest_insert_product', $post, $request, true );
	...
	public function update_item( $request ) {
		...
		try {
			 ...
			 * @param boolean         $creating  True when creating item, false when updating.
			 */
			do_action( 'poocommerce_rest_insert_product', $post, $request, false );
			$request->set_param( 'context', 'edit' );
			$response = $this->prepare_item_for_response( $post, $request );
	...
	protected function save_variations_data( $product, $request ) {
		foreach ( $request['variations'] as $menu_order => $data ) {
			...
			if ( ! $variation->get_slug() ) {
				...
				$variation->set_name( sprintf( __( 'Variation #%1$s of %2$s', 'poocommerce' ), $variation->get_id(), $product->get_name() ) );
			...
			do_action( 'poocommerce_rest_save_product_variation', $variation->get_id(), $menu_order, $data );
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! empty( $post->post_type ) && 'product_variation' === $post->post_type && 'product' === $this->post_type ) {
			...
			return new WP_Error( "poocommerce_rest_{$this->post_type}_invalid_id", __( 'Invalid post ID.', 'poocommerce' ), array( 'status' => 404 ) );
		}
		...
		$schema               = array(
			...
			'properties' => array(
				...
				'date_on_sale_from' => array(
					'description' => __( 'Start date of sale price.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'date_on_sale_to' => array(
					'description' => __( 'End date of sale price.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'price_html' => array(
				...
				'download_type' => array(
					'description' => __( 'Download type, this controls the schema on the front-end.', 'poocommerce' ),
					'type'        => 'string',
					'default'     => 'standard',
					'enum'        => array( 'standard' ),
					'context'     => array( 'view', 'edit' ),
				...
				'upsell_ids' => array(
					'description' => __( 'List of upsell products IDs.', 'poocommerce' ),
					'type'        => 'array',
					'items'       => array(
						'type'    => 'integer',
					),
				...
				'variations' => array(
					'description' => __( 'List of variations.', 'poocommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type'       => 'object',
						'properties' => array(
							'id' => array(
								'description' => __( 'Variation ID.', 'poocommerce' ),
								'type'        => 'integer',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							...
							'visible' => array(
								'description' => __( 'If the variation is visible.', 'poocommerce' ),
								'type'        => 'boolean',
								'context'     => array( 'view', 'edit' ),
							),
							'virtual' => array(
	...
	public function get_collection_params() {
		...
		$params['attribute'] = array(
			'description'       => __( 'Limit result set to products with a specific attribute.', 'poocommerce' ),
			'type'              => 'string',
		...
		$params['sku'] = array(
			'description'       => __( 'Limit result set to products with a specific SKU.', 'poocommerce' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
		);
