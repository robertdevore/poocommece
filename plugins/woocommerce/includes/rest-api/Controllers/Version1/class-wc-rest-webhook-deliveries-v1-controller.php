...
class WC_REST_Webhook_Deliveries_V1_Controller extends WC_REST_Controller {
	...
	public function register_routes() {
		register_rest_route( $this->namespace, '/' . $this->rest_base, array(
			'args' => array(
				'webhook_id' => array(
					'description' => __( 'Unique identifier for the webhook.', 'poocommerce' ),
					'type'        => 'integer',
				),
			),
			array(
	...
	public function get_items( $request ) {
		...
		if ( empty( $webhook ) || is_null( $webhook ) ) {
			return new WP_Error( 'poocommerce_rest_webhook_invalid_id', __( 'Invalid webhook ID.', 'poocommerce' ), array( 'status' => 404 ) );
		}
