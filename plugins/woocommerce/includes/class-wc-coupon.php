 ...
 * PooCommerce coupons.
 *
 * The PooCommerce coupons class gets coupon data from storage and checks coupon validity.
 *
...
class WC_Coupon extends WC_Legacy_Coupon {
	...
	public function __construct( $data = '' ) {
		...
		$coupon = apply_filters( 'poocommerce_get_shop_coupon_data', false, $data, $this );
	...
	protected function get_hook_prefix() {
		return 'poocommerce_coupon_get_';
	}
	...
	*/
	...
	/**
	 * If the filter is added through the poocommerce_get_shop_coupon_data filter, it's virtual and not in the DB.
	 *
	 * @since 3.2.0
	 * @param  string $context What the value is for. Valid values are 'view' and 'edit'.
	 * @return boolean
	...
	public function get_discount_amount( $discounting_amount, $cart_item = null, $single = false ) {
		...
		if ( $this->is_type( array( 'percent' ) ) ) {
			 ...
			 *
			 * Uses price inc tax if prices include tax to work around https://github.com/poocommerce/poocommerce/issues/7669 and https://github.com/poocommerce/poocommerce/issues/8074.
			 */
		...
		return apply_filters(
			'poocommerce_coupon_get_discount_amount',
	...
	*/
	...
	public function set_discount_type( $discount_type ) {
		...
		if ( ! in_array( $discount_type, array_keys( wc_get_coupon_types() ), true ) ) {
			$this->error( 'coupon_invalid_discount_type', __( 'Invalid discount type', 'poocommerce' ) );
		}
	...
	public function set_amount( $amount ) {
		...
		if ( $amount < 0 ) {
			$this->error( 'coupon_invalid_amount', __( 'Invalid discount amount', 'poocommerce' ) );
		}
	...
	public function set_email_restrictions( $emails = array() ) {
		...
		foreach ( $emails as $email ) {
			if ( ! is_email( $email ) ) {
				$this->error( 'coupon_invalid_email_address', __( 'Invalid email address restriction', 'poocommerce' ) );
			}
		}
	...
	*/
	...
	public function is_valid_for_cart() {
		return apply_filters( 'poocommerce_coupon_is_valid_for_cart', $this->is_type( wc_get_cart_coupon_types() ), $this );
	}
	...
	public function is_valid_for_product( $product, $values = array() ) {
		if ( ! $this->is_type( wc_get_product_coupon_types() ) || ! is_a( $product, WC_Product::class ) ) {
			return apply_filters( 'poocommerce_coupon_is_valid_for_product', false, $product, $this, $values );
		}
		...
		return apply_filters( 'poocommerce_coupon_is_valid_for_product', $valid, $product, $this, $values );
	}
	...
	public function get_coupon_message( $msg_code ) {
		switch ( $msg_code ) {
			case self::WC_COUPON_SUCCESS:
				$msg = __( 'Coupon code applied successfully.', 'poocommerce' );
				break;
			case self::WC_COUPON_REMOVED:
				$msg = __( 'Coupon code removed successfully.', 'poocommerce' );
				break;
			default:
				$msg = '';
				break;
		}
		return apply_filters( 'poocommerce_coupon_message', $msg, $msg_code, $this );
	}
	...
	public function get_coupon_error( $err_code ) {
		switch ( $err_code ) {
			case self::E_WC_COUPON_INVALID_FILTERED:
				$err = __( 'Coupon is not valid.', 'poocommerce' );
				break;
			case self::E_WC_COUPON_NOT_EXIST:
				/* translators: %s: coupon code */
				$err = sprintf( __( 'Coupon "%s" does not exist!', 'poocommerce' ), esc_html( $this->get_code() ) );
				break;
			case self::E_WC_COUPON_INVALID_REMOVED:
				/* translators: %s: coupon code */
				$err = sprintf( __( 'Sorry, it seems the coupon "%s" is invalid - it has now been removed from your order.', 'poocommerce' ), esc_html( $this->get_code() ) );
				break;
			case self::E_WC_COUPON_NOT_YOURS_REMOVED:
				// We check for supplied billing email. On shortcode, this will be present for checkout requests.
				$billing_email = \Automattic\PooCommerce\Utilities\ArrayUtil::get_value_or_default( $_POST, 'billing_email' ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
				if ( ! is_null( $billing_email ) ) {
					/* translators: %s: coupon code */
					$err = sprintf( __( 'Please enter a valid email to use coupon code "%s".', 'poocommerce' ), esc_html( $this->get_code() ) );
				} else {
					/* translators: %s: coupon code */
					$err = sprintf( __( 'Please enter a valid email at checkout to use coupon code "%s".', 'poocommerce' ), esc_html( $this->get_code() ) );
				}
				break;
			case self::E_WC_COUPON_ALREADY_APPLIED:
				$err = __( 'Coupon code already applied!', 'poocommerce' );
				break;
			case self::E_WC_COUPON_ALREADY_APPLIED_INDIV_USE_ONLY:
				/* translators: %s: coupon code */
				$err = sprintf( __( 'Sorry, coupon "%s" has already been applied and cannot be used in conjunction with other coupons.', 'poocommerce' ), esc_html( $this->get_code() ) );
				break;
			case self::E_WC_COUPON_USAGE_LIMIT_REACHED:
				$err = __( 'Coupon usage limit has been reached.', 'poocommerce' );
				break;
			case self::E_WC_COUPON_EXPIRED:
				$err = __( 'This coupon has expired.', 'poocommerce' );
				break;
			case self::E_WC_COUPON_MIN_SPEND_LIMIT_NOT_MET:
				/* translators: %s: coupon minimum amount */
				$err = sprintf( __( 'The minimum spend for this coupon is %s.', 'poocommerce' ), wc_price( $this->get_minimum_amount() ) );
				break;
			case self::E_WC_COUPON_MAX_SPEND_LIMIT_MET:
				/* translators: %s: coupon maximum amount */
				$err = sprintf( __( 'The maximum spend for this coupon is %s.', 'poocommerce' ), wc_price( $this->get_maximum_amount() ) );
				break;
			case self::E_WC_COUPON_NOT_APPLICABLE:
				$err = __( 'Sorry, this coupon is not applicable to your cart contents.', 'poocommerce' );
				break;
			case self::E_WC_COUPON_USAGE_LIMIT_COUPON_STUCK:
				if ( is_user_logged_in() && wc_get_page_id( 'myaccount' ) > 0 && ! WC()->is_store_api_request() ) {
					...
					$err = sprintf( __( 'Coupon usage limit has been reached. If you were using this coupon just now but your order was not complete, you can retry or cancel the order by going to the <a href="%s">my account page</a>.', 'poocommerce' ), wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ) );
				} else {
					$err = $this->get_coupon_error( self::E_WC_COUPON_USAGE_LIMIT_REACHED );
				}
				break;
			case self::E_WC_COUPON_USAGE_LIMIT_COUPON_STUCK_GUEST:
				$err = __( 'Coupon usage limit has been reached. Please try again after some time, or contact us for help.', 'poocommerce' );
				break;
			case self::E_WC_COUPON_EXCLUDED_PRODUCTS:
				...
				$err = sprintf( __( 'Sorry, this coupon is not applicable to the products: %s.', 'poocommerce' ), implode( ', ', $products ) );
				break;
			case self::E_WC_COUPON_EXCLUDED_CATEGORIES:
				...
				$err = sprintf( __( 'Sorry, this coupon is not applicable to the categories: %s.', 'poocommerce' ), implode( ', ', array_unique( $categories ) ) );
				break;
			case self::E_WC_COUPON_NOT_VALID_SALE_ITEMS:
				$err = __( 'Sorry, this coupon is not valid for sale items.', 'poocommerce' );
				break;
			default:
				$err = '';
				break;
		}
		return apply_filters( 'poocommerce_coupon_error', $err, $err_code, $this );
	}
	...
	public static function get_generic_coupon_error( $err_code ) {
		switch ( $err_code ) {
			case self::E_WC_COUPON_NOT_EXIST:
				$err = __( 'Coupon does not exist!', 'poocommerce' );
				break;
			case self::E_WC_COUPON_PLEASE_ENTER:
				$err = __( 'Please enter a coupon code.', 'poocommerce' );
				break;
			default:
				$err = '';
				break;
		...
		return apply_filters( 'poocommerce_coupon_error', $err, $err_code, null );
	}
	...
	public function get_context_based_coupon_errors( $err_code = null ) {
		...
		switch ( $err_code ) {
			case self::E_WC_COUPON_NOT_YOURS_REMOVED:
				return array(
					/* translators: %s: coupon code */
					'cart'     => sprintf( __( 'Please enter a valid email at checkout to use coupon code "%s".', 'poocommerce' ), esc_html( $this->get_code() ) ),
					/* translators: %s: coupon code */
					'checkout' => sprintf( __( 'Please enter a valid email to use coupon code "%s".', 'poocommerce' ), esc_html( $this->get_code() ) ),
				);
			default:
				return array();
		}
