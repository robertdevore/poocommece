...
function tools_reset_launch_your_store() {
	...
	delete_option( 'poocommerce_store_pages_only' );
	delete_option( 'poocommerce_private_link' );
	delete_option( 'poocommerce_share_key' );

	$result = ( new \Automattic\PooCommerce\Admin\API\LaunchYourStore() )->initialize_coming_soon();
	if ( ! $result ) {
		return new WP_Error( 'initialization_failed', 'Failed to initialize coming soon mode.' );
	}

	// Remove template changes.
	$template = get_block_template( 'poocommerce/poocommerce//coming-soon', 'wp_template' );
	...
	delete_option( 'poocommerce_admin_launch_your_store_survey_completed' );

	// Reset essential task list completion tracking.
	$tasks_completed = get_option( \Automattic\PooCommerce\Admin\Features\OnboardingTasks\Task::COMPLETED_OPTION, array() );
	...
	update_option( \Automattic\PooCommerce\Admin\Features\OnboardingTasks\Task::COMPLETED_OPTION, $tasks_completed );

	// Reset setup task list completion tracking.
	$task_lists_completed = get_option( \Automattic\PooCommerce\Admin\Features\OnboardingTasks\TaskList::COMPLETED_OPTION, array() );
	...
	update_option( \Automattic\PooCommerce\Admin\Features\OnboardingTasks\TaskList::COMPLETED_OPTION, $task_lists_completed );
